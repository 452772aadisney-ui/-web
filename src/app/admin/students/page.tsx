...
import { fetchStudentLastAccessMap, formatLastAccessedAt } from '@/lib/auth/last-access'
...
export default async function AdminStudentsPage() {
  ...
  const [students, gradeTagByStudentId, lastAccessByStudentId] = await Promise.all([
    fetchStudentList(),
    fetchGradeTagNamesByStudentId(),
    fetchStudentLastAccessMap(),
  ])

  const studentsWithLastAccess = students.map((student) => ({
    ...student,
  ...
  return (
    <AdminPageShell title="生徒一覧" backHref="/admin" backLabel="管理画面">
      <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        ...
        {studentsWithLastAccess.length === 0 ? (
