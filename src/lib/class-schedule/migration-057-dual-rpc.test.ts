...
describe('057 dual-RPC and verify artifacts', () => {
  ...
  it('keeps both overloads during bridge and documents zero-downtime order', () => {
    ...
    expect(precheck).toMatch(/sessions_off_five_minute_grid|time_out_of_range/)
    expect(precheck).toMatch(/notify_revision_sum/)
    expect(precheck).toMatch(/to_jsonb\(d\)\s*->>\s*'location_details'/)
    expect(precheck).toMatch(/ready_for_057/)
