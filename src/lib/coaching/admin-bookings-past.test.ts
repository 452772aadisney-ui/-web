...
describe('past coaching bookings search and pagination wiring', () => {
  ...
  it('wires pastPage, pastQ, edit toggle for today/past, always actions for future', () => {
    ...
    expect(page).toContain('isKisotsuGradeTag')
    ...
    expect(ui).toContain('actionCharcoalClass')
    expect(ui).toContain('bg-slate-700')
    expect(ui).toContain('hover:bg-slate-800')
    expect(ui).toContain('actionBlueClass')
    expect(ui).toContain('actionRedClass')
    expect(ui).toContain('actionGrayClass')
    expect(ui).toContain('ml-auto min-w-0 max-w-full')
    expect(ui).toContain('justify-end gap-2')
