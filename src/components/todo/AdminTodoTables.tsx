...
import type { HomeworkCompletion } from '@/types/todo'
...
export function AdminStudentTodoTable({
  homework,
  completions,
}: AdminStudentTodoTableProps) {
  const completedIds = new Set(completions.map((c) => c.homework_task_id))

  if (homework.length === 0) {
    return <p className="text-sm text-muted">登録されている宿題はありません。</p>
  }

  const incompleteCount = homework.filter((t) => !completedIds.has(t.id)).length
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-muted">
        ...
        {' / '}
        完了: <span className="font-bold text-green-700">{homework.length - incompleteCount}</span> 件
      </p>
            ...
            {homework.map((task) => {
              ...
              return (
                <tr key={task.id}>
                  ...
                  <td className="py-3 pr-4 text-muted">{task.subject}</td>
                  <td className="py-3 pr-4 text-muted">{formatDate(task.due_date)}</td>
                  <td className="py-3">
                    {done ? (
